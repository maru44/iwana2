...
def home(request):
    posts = Wanted.objects.prefetch_related('plat').select_related('user').order_by('-posted')
    context = {
        "posts": posts,
    }
...
def det_wanted(request, slug):
    post = get_object_or_404(Wanted.objects.prefetch_related('plat').select_related('user'), slug=slug)
    #post = Wanted.objects.prefetch_related('plat').select_related('user').get(slug=slug)
    offers = Offer.objects.select_related('wanted').select_related('user').filter(wanted=post).order_by('-posted')
    context = {
        "post": post,
        "offers": offers,
        "now": timezone.now(),
...
def update_wanted(request, slug):
    wanted = get_object_or_404(Wanted.objects.prefetch_related("plat").select_related('user'), slug=slug)
    plats = Plat.objects.all()
    ...
    if wanted.user == request.user:
        if request.method == "POST":
            form = WantedForm(request.POST, request.FILES, instance=wanted)# instance is wanted
            #form = WantedForm(request.POST, request.FILES)
            if form.is_valid():
                post = form.save(commit=False)
...
def delete_wanted(request, slug):
    wanted = get_object_or_404(Wanted.objects.prefetch_related('plat').select_related(), slug=slug)
...
def users_wanted(request, username):
    ...
    wanted = Wanted.objects.prefetch_related('plat').select_related('user').filter(user=target).order_by('-posted')
    context = {
        "target": target,
        "posts": wanted,
    }
